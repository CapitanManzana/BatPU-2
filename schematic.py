...
def make_schematic(mc_filename, schem_filename, output_path):
    mc_file = open(mc_filename, 'r')
    schem = mcschematic.MCSchematic()
    
    # Generate 1024 xz positions
    
    # Generar posiciones rotadas 90º
    mem_start_pos = [2, -1, 4] # Invertimos el inicio (X, Y, Z)
    pos_list = []
    ...
    for i in range(2):
        for j in range(32):
            ...
            if i == 1:
                pos[2] += 2 # Antes era pos[0] -= 2 (Eje X -> Z)

            pos[0] += 2 * j # Antes era pos[2] += 2 * j (Eje Z -> X)
            if j >= 16:
                pos[0] += 4
            ...
            for k in range(16):
                ...
                if k % 2 == 0:
                    pos[2] += 7 # Desplazamiento de columna (Eje X -> Z)
                    pos[0] += 1 if j < 16 else -1 # Ajuste fino (Eje Z -> X)
                else:
                    pos[2] += 7
                    pos[0] -= 1 if j < 16 else -1
    ...
    for address, line in enumerate(lines):
        ...
        face = 'north' if address < 512 else 'south'
    ...
    schem.save(output_path, schem_filename, version=mcschematic.Version.JE_1_18_2)
